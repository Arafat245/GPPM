...
class DataGenerator(object):
    def __init__(self, Nbiom, interval, param, Nsubs, seed = None):
        self.Nbiom = Nbiom
        self.YData = []
        self.YDataNoNoise = []
        self.XData = []
    ...
    def get_df(self):
        ...
        for i in range(len(self.ZeroXData[0])): 
            for t in range(len(self.ZeroXData[0][i])): 
                df = pd.concat([df, {'RID':int(i),'time':self.ZeroXData[0][i][t], 'time_gt':self.XData[0][i][t]}], ignore_index=True)  
        ...
        for b in range(self.Nbiom): 
            for i in range(len(self.ZeroXData[b])): 
                for t_i, t in enumerate(self.ZeroXData[b][i]):
                    df.loc[(df['RID'] == int(i)) & (df['time'] == t),'biom_'+str(b)] = self.YData[b][i][t_i]       
        return(df)
    ...
    def plot(self, mode):
        ...
        if mode == 'long':
            ...
            for i in np.arange(self.Nbiom):
                ...
                plt.plot( range(len(self.model[i])),self.model[i],  color=Blues(i * 2), linewidth=3, label = "biom.  " + str(i))
